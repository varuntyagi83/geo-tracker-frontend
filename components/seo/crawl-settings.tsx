...
export function CrawlSettings({ maxPages, maxDepth, onChange, disabled }: CrawlSettingsProps) {
  function setPreset(name: keyof typeof PRESETS) {
    ...
    onChange({ maxPages: preset.maxPages, maxDepth: preset.maxDepth })
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {(Object.keys(PRESETS) as Array<keyof typeof PRESETS>).map(name => {
          const preset = PRESETS[name]
          const isActive = maxPages === preset.maxPages && maxDepth === preset.maxDepth
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-cyan-500 dark:text-cyan-400 font-mono font-semibold">
            {maxPages} pages
          </span>
        </div>
        <Slider
