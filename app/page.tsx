...
function CTASection() {
  ...
  // Backend API URL - this is now the primary submission endpoint
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Submit directly to our backend (Formspree removed)
      // Backend handles:
      // 1. Saving lead to database
      // 2. Sending acknowledgment email to lead
      // 3. Sending notification email to admin(s)
      ...
      // Success - lead was captured (even if email failed)
      if (data.success) {
        setSubmitted(true);
      } else {
