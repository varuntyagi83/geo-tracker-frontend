...
import type { SiteAnalysis, AIRecommendations, Recommendation, PageRecommendation, ExecutiveSummary } from '@/types/seo/analysis'
...
function buildUserPrompt(analysis: SiteAnalysis): string {
  ...
  // Full page data — truncated to prevent prompt injection and runaway prompt sizes
  const allPagesData = allPagesSorted.map(p => {
    const safeUrl = p.url.slice(0, 150)
    const issues = [
      ...p.meta.filter(c => c.status !== 'pass').map(c => `meta/${c.tag}: ${String(c.found).slice(0, 200)}`),
      ...p.content.filter(c => c.status !== 'pass').map(c => `content/${c.tag}: ${String(c.found).slice(0, 200)}`),
      ...p.technical.filter(c => c.status !== 'pass').map(c => `technical/${c.tag}: ${String(c.found).slice(0, 200)}`),
      ...p.aeo.filter(c => c.status !== 'pass').map(c => `aeo/${c.tag}: ${String(c.found).slice(0, 200)}`),
    ]
    return `URL: ${safeUrl}
...
export async function generateRecommendations(analysis: SiteAnalysis): Promise<AIRecommendations | null> {
  ...
  try {
    ...
    console.error('[SEO/openai] generateRecommendations failed:', err)
    return null
  }
}
